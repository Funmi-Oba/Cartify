<template>
  ...
  >
    ...
    <ol class="flex items-center gap-1 text-base">
      <li>
        <router-link to="/" class="transition-colors text-[#666] text-sm md:text-base">
          Home
        </router-link>
      </li>
      <li v-for="(crumb, index) in crumbs" :key="index" class="flex items-center gap-1">
        ...
        <router-link
          v-if="crumb.name"
          :to="{ name: crumb.name }"
          class="transition-colors text-[#999] text-sm md:text-base"
        >
          {{ crumb.label }}
        </router-link>
        <span v-else class="text-[#999] text-sm md:text-base">{{ crumb.label }}</span>
      </li>
    </ol>
  </nav>
</template>
